...
class DistressedProAdapter(IngestionAdapter):
    """Fetches REO/bank-owned property listings from DistressedPro.com."""

    source_id = "distressed_pro"
    # Maine bank-owned properties on DistressedPro
    SEARCH_URL = "https://www.distressedpro.com/app/properties/ME"
        ...
        try:
            with sync_playwright() as p:
                ...
                items = page.query_selector_all(
                    ...
                    ".property-card, .home-card, table tr"
                )

                if not items:
                    # Try looking for links that might be listings
                    items = page.query_selector_all("a[href*='/properties'], a[href*='/banks'], a[href*='/property']")
                ...
                for item in items[:100]:  # Limit to first 100 listings
                    try:
                        ...
                        if url.startswith("/"):
                            url = f"https://www.distressedpro.com{url}"
                        ...
                        bank_el = item.query_selector(".bank, [class*='bank']")
                        bank = bank_el.inner_text().strip() if bank_el else ""
