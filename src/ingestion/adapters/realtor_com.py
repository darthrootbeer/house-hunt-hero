...
class RealtorComAdapter(IngestionAdapter):
    """Fetches listings from Realtor.com (Maine).
    
    Note: Realtor.com has anti-bot measures. This adapter may require refinement
    for production use.
    """

    source_id = "realtor_com"
    BASE_URL = "https://www.realtor.com"
    # Maine for-sale listings
    SEARCH_URL = "https://www.realtor.com/realestateandhomes-search/Maine"
    ...
    def fetch(self) -> List[RawListing]:
        ...
        try:
            with sync_playwright() as p:
                ...
                # Realtor.com uses property cards
                items = page.query_selector_all("li[data-testid='result-card'], .card-content, [class*='PropertyCard']")

                for item in items[:100]:
                    try:
                        link_elem = item.query_selector("a[href*='/realestateandhomes-detail/']")
                        if not link_elem:
                            continue
                        ...
                        title = "Property"
                        try:
                            address_elem = item.query_selector("[data-testid='card-address'], .address, [class*='Address']")
                            if address_elem:
