...
class MaloneCommercialAdapter(IngestionAdapter):
    """Fetches listings from Malone Commercial Brokers (Maine commercial properties)."""

    source_id = "malone_commercial"
    BASE_URL = "https://www.malonecb.com"
    # All Maine commercial properties search page
    SEARCH_URL = "https://www.malonecb.com/search-listings#/search/grid/"
    ...
    def fetch(self) -> List[RawListing]:
        ...
        try:
            with sync_playwright() as p:
                ...
                # Wait for listings to load (single-page app with hash routing)
                page.wait_for_timeout(6000)

                # Malone uses property cards - look for links to property details
                ...
                for item in items[:100]:  # Limit to first 100 listings
                    try:
                        ...
                        # Extract property info from card
                        link_text = item.inner_text().strip()
                        
                        # Try to get full card context
                        ...
                        try:
                            ...
                            if card_element:
                                ...
                                # Look for property type labels
                                for line in lines:
                                    line_lower = line.lower()
                                    if any(t in line_lower for t in ['multi-family', 'multifamily', 'hospitality', 'industrial', 'retail', 'office', 'land', 'mixed use']):
                                        property_type = line
                                        break
                        except:
                            pass
